...
def _thinningIteration(im, iter):
    # Convert input image to uint8 to avoid float64 issues
    I = im.astype(np.uint8).copy()
    ...
    for i in range(1, im.shape[0] - 1):
        for j in range(1, im.shape[1] - 1):
            ...
            p8 = I[i, j-1]
            p9 = I[i-1, j-1]
            A  = (p2 == 0 and p3 == 1) + (p3 == 0 and p4 == 1) + (p4 == 0 and p5 == 1) + \
    ...
    # Perform the bitwise operation with consistent data types (uint8)
    return (I & ~M).astype(np.uint8)
...
def lines_between_ends(ends):
    ...
    for i in range(ends[0].size):
        ...
        for j in range(i+1, ends[0].size):  # (i != j and j > i) => range(i+1, ...)
            ...
            if (75 < angle < 105) and (5 < dist < 100):
                q1 = np.where(skel[miny-5:maxy+5, minx-5:maxx+5] == 255)
                point_count = 0
                for k in range(len(q1[0])):
                    test_pt = np.array([minx - 5 + q1[1][k], miny - 5 + q1[0][k]])
                    line_pts = np.array([[x1, y1], [x2, y2]])
                    d1 = point_to_line_dist(test_pt, line_pts)
                    if d1 < 2:
                        point_count += 1
            ...
            elif ((angle < 15) or (angle > 170)) and (5 < dist < 100):
                q1 = np.where(skel[miny-5:maxy+5, minx-5:maxx+5] == 255)
                point_count = 0
                for k in range(len(q1[0])):
                    test_pt = np.array([minx - 5 + q1[1][k], miny - 5 + q1[0][k]])
    ...
    # Force both returns to have shape (N, 4), even if empty
...
def box_between_ends(lines):
    ...
    lookup = np.zeros((len(lines),len(lines)),dtype=int)
    lookup = lookup-1
